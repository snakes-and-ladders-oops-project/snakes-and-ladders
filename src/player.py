...
class Player:
    ...
    def draw_token(self, screen):
        ...
        # Create a transparent surface for the token
        token_surface = pygame.Surface((2 * self.radius, 2 * self.radius), pygame.SRCALPHA)
        pygame.draw.circle(token_surface, self.token_color, (self.radius, self.radius), self.radius)

        # Blit the transparent token surface onto the screen
        screen.blit(token_surface, (center_x - self.radius, center_y - self.radius))
